...
def check_image_with_sightengine(image_url: str) -> dict:
    ...
    api_url = 'https://api.sightengine.com/1.0/check.json' # Đã sửa https://
...
def handle_photo(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if result.get('status') == 'success':
            # === LOGIC ĐỌC KẾT QUẢ ĐÃ SỬA THEO JSON CỦA BẠN ===
            # 1. Lấy đối tượng 'type'
            type_info = result.get('type') 

            if type_info:
                # 2. Lấy điểm 'ai_generated' từ bên trong 'type'
                prob_ai_float = type_info.get('ai_generated')
                ...
                if prob_ai_float is not None:
                    ...
                    update.message.reply_text("Không thể phân tích ảnh này (API không trả về điểm 'ai_generated' bên trong 'type').")
            else:
                # Đây là lỗi bạn đang gặp phải
                update.message.reply_text("Không thể phân tích ảnh này (API không trả về kết quả 'type').")
            # === KẾT THÚC SỬA LỖI ===
        else:
...
def webhook():
    ...
    json_data = request.get_json(force=True) # Đã sửa 'force_True' thành 'force=True'
