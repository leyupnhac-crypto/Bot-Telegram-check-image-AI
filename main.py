...
def check_image_with_sightengine(image_url: str) -> dict:
    ...
    # === THAM SỐ ĐÃ BỎ 'opt_beta_models' ===
    params = {
        'models': 'genai',           # Model 'genai' theo hình bạn gửi
        'url': image_url,
...
def handle_photo(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        if result.get('status') == 'success':
            ai_info = result.get('genai') # Đọc kết quả từ model 'genai'
            ...
            if ai_info:
                ...
                update.message.reply_text("Không thể phân tích ảnh này (API không trả về kết quả 'genai').")
        else:
