...
def check_image_with_aiornot(image_url: str) -> dict:
    """Gửi URL ảnh đến AIorNot để kiểm tra."""
...
def handle_photo(update: Update, context: CallbackContext) -> None:
    """Xử lý ảnh người dùng gửi (ĐÃ CẬP NHẬT ĐỂ HIỂN THỊ ĐẦY ĐỦ)."""
    try:
        ...
        if report:
            ...
            # 2. Tìm tên Generator AI có điểm cao nhất
            generator_info = report.get('generator', {})
            generator_name = "Không rõ"
            if generator_info:
                try:
                    # Tìm key (tên) có giá trị 'confidence' cao nhất
                    generator_name = max(generator_info.items(), key=lambda item: item[1].get('confidence', 0))[0]
                    # Làm đẹp tên (ví dụ: stable_diffusion -> Stable Diffusion)
                    generator_name = generator_name.replace("_", " ").title()
                except ValueError:
                    generator_name = "Không rõ" # Xảy ra nếu generator_info rỗng

            # 3. Lấy các kết quả phụ
