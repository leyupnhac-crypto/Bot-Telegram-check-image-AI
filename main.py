...
def check_image_with_sightengine(image_url: str) -> dict:
    ...
    params = {
        'models': 'ai-generated',     # Đổi lại model chúng ta muốn
        'opt_beta_models': 'true',     # <-- THAM SỐ CÒN THIẾU
        'url': image_url,
...
def handle_photo(update: Update, context: CallbackContext) -> None:
    ...
    try:
        ...
        image_url = photo_file.file_path # Đây là URL đầy đủ, đã đúng
        ...
        if result.get('status') == 'success':
            ...
            if ai_info:
                ...
                # Lỗi này có thể xảy ra nếu model 'nudity' được dùng
                update.message.reply_text("Không thể phân tích ảnh này (API không trả về kết quả 'ai_generated').")
        else:
