...
# --- Các hàm xử lý của Bot (ĐÃ SỬA LỖI 500) ---

def check_image_with_aiornot(image_data: bytes) -> dict:
    """Tải DỮ LIỆU ảnh lên AIorNot v2 (đúng cách) để kiểm tra."""
    
    # Dùng endpoint v2/sync, được thiết kế để UPLOAD file
    api_url = 'https://api.aiornot.com/v2/image/sync'
    
    headers = {
        'Authorization': f"Bearer {AIORNOT_API_KEY}",
        'Accept': 'application/json'
        # Không cần 'Content-Type', requests sẽ tự xử lý
    }
    
    # === SỬA LỖI: Gửi dữ liệu ảnh dưới dạng 'files' (multipart/form-data) ===
    # 'image' là tên trường (field) mà API v2 yêu cầu
    payload_files = {
        'image': image_data
    }

    try:
        response = requests.post(api_url, headers=headers, files=payload_files)
...
def handle_photo(update: Update, context: CallbackContext) -> None:
    """Xử lý ảnh người dùng gửi (Đọc kết quả v2)."""
    try:
        # 1. Lấy đối tượng file từ Telegram
        photo_file = update.message.photo[-1].get_file()

        # 2. TẢI DỮ LIỆU ảnh về bộ nhớ (thay vì chỉ lấy link)
        # .download_as_bytearray() sẽ trả về dữ liệu thô (bytes) của ảnh
        image_bytes = photo_file.download_as_bytearray()

        waiting_message = update.message.reply_text("Đang phân tích ảnh (đang tải lên)...")
        
        # 3. Gửi DỮ LIỆU (bytes) đi, thay vì gửi link
